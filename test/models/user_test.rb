...
class UserTest < ActiveSupport::TestCase
  context "validations" do
    ...
    should "can't be blank if record is new" do
      user = Fabricate.build(:user, password: "")
      assert_no_difference("User.count") {user.save}
    end

    should "skip validation if records is not new and password is blank" do
      Fabricate(:user, password: "foobar!23")
      user = User.last
      assert user.valid?
    end

    should "validate the password if the record is no new an the password is not blank" do
  ...
  context ".authenticate" do
    ...
    context "when password is not ok" do
      ...
      should "return false" do
        assert_not User.authenticate(email: "foobar@foo.com", password: "fsdfjasd23")
      end
    end
  end
end
